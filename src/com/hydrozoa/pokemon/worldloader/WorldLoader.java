...
import com.hydrozoa.pokemon.model.WildGrassTile;
...
public class WorldLoader extends AsynchronousAssetLoader<World, WorldLoader.WorldParameter> {
	...
	private TextureRegion wildGrassTexture;
	...
	public void loadAsync(AssetManager asman, String filename, FileHandle file, WorldParameter parameter) {
		...
		wildGrassTexture = atlas.findRegion("wild_grass");
		...
		try {
			while (reader.ready()) {
				...
				case "addTree":
					...
					break;
				case "addWildGrass":
					addWildGrass(tokens[1], tokens[2]);
					break;
				case "addDoor":
	...
	private void addWildGrass(String sx, String sy) {
		int x = Integer.parseInt(sx);
		int y = Integer.parseInt(sy);

		// WildGrassTile
		LTerrain t = world.getMap().getTile(x, y).getTerrain();
		WildGrassTile wildGrassTile = new WildGrassTile(t, 0.3f); // 30% encounter chance
		world.getMap().setTile(wildGrassTile, x, y);

		GridPoint2[] gridArray = new GridPoint2[1];
		gridArray[0] = new GridPoint2(0,0);
		WildGrass wildGrass = new WildGrass(x, y, wildGrassTexture, 1f, 1f, gridArray);
		world.addObject(wildGrass);
	}
